...
## Следующие шаги
1. **Увеличить max_upload_size** на хостинге (php.ini: upload_max_filesize = 256M, post_max_size = 256M)
2. **Платная подписка** — ЮKassa + Stripe, WebView оплата
3. **Чат** — общение с автором
4. **Push-уведомления** — оповещения о новом контенте
5. **Детальный экран статьи** — вывести обложку (image_url из content_data) над телом контента
6. **Автозаполнение категорий** — при вводе категории в админке показывать подсказки из уже существующих категорий (как в Яндексе)
