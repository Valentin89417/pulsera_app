...
#### Фаза 1: БД
- [ ] Добавить колонку `sender` ('user' | 'author') в `chat_messages`
- [ ] RLS: админ видит все сообщения и может отвечать
- [ ] Включить Realtime для `chat_messages`

#### Фаза 2: API
- [ ] Обновить `sendChatMessage` — параметр `sender`
- [ ] `getAdminChatUsers()` — список пользователей с последним сообщением
- [ ] `getAdminChatMessages(userId)` — все сообщения диалога
- [ ] `markChatAsRead(userId)` — отметить как прочитанное

#### Фаза 3: Пользовательский чат
- [ ] `src/app/chat.tsx` — экран чата (Telegram-стиль)
- [ ] Лента сообщений (FlatList inverted), сообщения пользователя справа, автора слева
- [ ] Поле ввода + кнопка отправки
- [ ] Realtime подписка на новые сообщения
- [ ] Блокировка для не-подписчиков

#### Фаза 4: Админ-панель
- [ ] `src/app/admin/chat.tsx` — список чатов с пользователями
- [ ] `src/app/admin/chat/[userId].tsx` — диалог с пользователем
- [ ] Бейдж непрочитанных

#### Фаза 5: Навигация
- [ ] Кнопка «Чат с автором» в профиле → `/chat`
- [ ] Кнопка «Чат с пользователями` в админке → `/admin/chat`
...
#### Фаза 1: БД
- [ ] Добавить колонки в `profiles`: `phone`, `bio`, `location`
- [ ] Миграция для существующих данных

#### Фаза 2: Экран редактирования
- [ ] `src/app/profile-edit.tsx` — форма редактирования профиля
- [ ] Поля: аватар (загрузка в WP Media Library), имя, email, телефон, bio, локация
- [ ] Загрузка аватара через `pickFile` + `uploadFile`
- [ ] Валидация (email, телефон)

#### Фаза 3: Интеграция
- [ ] Кнопка редактирования в профиле (иконка карандаша)
- [ ] Обновление `updateProfile` в API
...
3. **Чат с автором** — Telegram-стиль, доступен для подписки «Пробуждение»
4. **Настройка профиля** — редактирование аватара, имени, email, телефона
5. **Push-уведомления** — оповещения о новом контенте
